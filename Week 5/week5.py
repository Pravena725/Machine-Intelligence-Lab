...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        first_op = self.history[1]
        second_op = self.history[2]
        first_op.grad=np.zeros_like(first_op.arr) #Returns arr of 0s with same shape and type as a given array
        second_op.grad=np.zeros_like(second_op.arr)
                
        if first_op.requires_grad: 
            first_op.grad += np.ones_like(first_op.arr) #returns arr of 1s of same shape,type as given array
            
        if second_op.requires_grad: 
            second_op.grad += np.ones_like(second_op.arr)
            
        if gradients is None:
            return (first_op.grad,second_op.grad) #directly returns it
            
        if first_op.requires_grad: 
            first_op.grad = np.multiply(np.ones_like(first_op.arr), gradients) #multiply wrt given gradients
            
        if second_op.requires_grad: 
            second_op.grad = np.multiply(np.ones_like(second_op.arr), gradients)
            
        grad1=first_op.grad
        grad2=second_op.grad
    ...
    def grad_matmul(self, gradients=None):
        ...
        if gradients is None:
            if first_op.requires_grad:
                first_op.grad += np.matmul(np.ones_like(first_op.arr), second_op.arr.transpose())
            if second_op.requires_grad:
                temp = np.matmul(np.ones_like(second_op.arr), first_op.arr)
                second_op.grad += temp.transpose()
        		
        else:
            if first_op.requires_grad:
                first_op.grad += np.multiply(np.matmul(np.ones_like(first_op.arr), second_op.arr.transpose()), gradients)
            if second_op.requires_grad:
                second_op.grad += np.multiply(np.matmul(np.ones_like(second_op.arr), first_op.arr).transpose(), gradients)
