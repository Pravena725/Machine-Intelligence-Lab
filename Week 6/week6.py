...
class SVM:
    ...
    def solve(self):
        ...
        #first we have to preprocess and then use SVC        
        #we have to use StandardScaler as it gives max accuracy and we set C=1 to prevent overfit 
        SVCmodel=Pipeline([('scalar1',StandardScaler()),('svm_classifier1',SVC(C=1))])
        
        SVCmodel.fit(self.X,self.y) #pass the features, target
        return SVCmodel 
