...
class AdaBoost:
    ...
    def stump_error(self, y, y_pred, sample_weights):
        ...
        miss_classified = np.where( y_pred!=y,1,0) #returns indices of elements of input array where condn is satisfied
        stump_err_res = np.sum ( miss_classified * sample_weights ) / np.sum( sample_weights )
        return stump_err_res 
    ...
    def compute_alpha(self, error):
        ...
        eps = 1e-9
        # TODO
        temp = np.log ((1-error) / (eps+error)) #eps is used to prevent ZeroDivisionError
        alpha = (0.5) * temp
        return alpha
    ...
    def update_weights(self, y, y_pred, sample_weights, alpha):
        ...
        if(error==0):
            res = np.exp (alpha*(np.not_equal(y, y_pred)).astype(int))
            return (sample_weights) * res   
    ...
    def predict(self, X):
        ...
        for i in range(self.n_stumps):
            output = np.array (self.stumps[i].predict(X))
