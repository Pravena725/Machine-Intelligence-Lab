...
class KMeansClustering:
    ...
    def e_step(self, data):
        ...
        for i in range(len(data)):
            for j in range(len(self.centroids)):
                diff = self.centroids[j]-data[i]
                l1.append(np.linalg.norm(diff)) #finds the norm of the diff
        cluster_assignment=[]

        l1_reshaped = np.reshape(l1,(len(data),len(self.centroids))) #without changing data it changes shape of matrix 

        for i in range(len(l1_reshaped)):
            min_ele = np.argmin(l1_reshaped[i]) #returns indices of the min element of l1_reshaped[i]
            cluster_assignment.append(min_ele) 
    ...
    def m_step(self, data, cluster_assgn):
        ...
        rows_m = len(self.centroids)
        cols_d = len(self.centroids[0])
        new_center = np.zeros(shape=(rows_m,cols_d)) #create a new matrix [rows_m x cols_d] filled with 0s
        ...
        for k in range(len(new_center)):
            count=(cluster_assgn == k).sum() #chks if equal and then computes sum
    ...
    def evaluate(self, data, cluster_assign):
        ...
        for i in range (len(cluster_assign)):
            temp = np.linalg.norm (data[i] - self.centroids[int(cluster_assign[i])]) ** 2
            sse = sse + temp;
